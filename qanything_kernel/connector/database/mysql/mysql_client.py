...
class KnowledgeBaseManager:
    ...
    def get_file_by_status(self, kb_ids, status):
        ...
        query = "SELECT file_id, file_name FROM File WHERE kb_id IN ({}) AND deleted = 0 AND status = ?".format(
            kb_ids_str)
    ...
    def delete_knowledge_base(self, user_id, kb_ids):
        ...
        # 更新文件的删除状态也需要使用参数化查询
        query = "UPDATE File SET deleted = 1 WHERE kb_id IN ({}) AND kb_id IN (SELECT kb_id FROM KnowledgeBase WHERE user_id = ?)".format(
            placeholders)
