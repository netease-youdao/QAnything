...
class OpenAILLM(ABC):
    ...
    def __init__(self, model_name, max_token):
        ...
        super().__init__()
        if model_name == 'youdao-local-llm-qanything-turbo':
            base_url = "http://10.55.162.153:8001/v1"
        elif model_name == 'youdao-local-llm-qanything-std':
            base_url = "http://10.55.162.135:8001/v1"
        else:
            base_url = "http://10.55.162.153:8001/v1"
        ...
        debug_logger.info(f"init OpenAILLM with model: {model_name}, max_token: {self.max_token}")
    ...
    def _call(self, prompt: str, history: List[List[str]], streaming: bool=False) -> str:
        ...
        try:
            ...
            debug_logger.info(f"Error in history {history}: {traceback.format_exc()}")
            ...
            if streaming:
                ...
                for event in response:
                    ...
                    if isinstance(event['choices'], List) and len(event['choices']) > 0:
                        ...
                        if event_text == "":
                            total_tokens = event['usage']['total_tokens'] if 'usage' in event else 0
                            prompt_tokens = event['usage']['prompt_tokens'] if 'usage' in event else 0
                            completion_tokens = event['usage']['completion_tokens'] if 'usage' in event else 0
                            delta = {'answer': "", 'total_tokens': total_tokens, 'prompt_tokens': prompt_tokens, 
                ...
                delta = {'answer': event_text, 'total_tokens': usage.get('total_tokens', 0),
...
"""
    ...
    final_result = ""
    for answer_result in llm.generatorAnswer(provider='vllm', prompt=prompt,
